...
const CLASS_NAME = 'output_CJSON rendered_html';
...
export function register_renderer(notebook, events, OutputArea) {
  /* A function to render output of 'application/cjson' mime type */
...
/**
 * Re-render cells with output data of 'application/cjson' mime type
 * on load notebook
 */
export function render_cells(notebook) {
  /* Get all cells in notebook */
  notebook.get_cells().forEach(cell => {
    /* If a cell has output data of 'application/cjson' mime type */
    if (
      cell.output_area &&
